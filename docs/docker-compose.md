...
| `SNAPSHOT_LIVE`                    | `auto`                          | policy for bare `/snapshot/<sn>`. `auto` uses battery capability; `1` forces live; `0` disables live. `?mode=auto`, `?mode=stored`, or `?mode=live` overrides it for one request. Retained/persisted imagery is used when live is skipped or fails                                                                                                                                                                                                                    |
